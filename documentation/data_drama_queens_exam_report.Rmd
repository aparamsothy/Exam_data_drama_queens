...
tot_obs <- patient_data %>% count() %>% pull() # Total observations
...
There are `r unique_obs` unique patient observations out of `r tot_obs` rows, indicating repeated measurements for some patients. Also `r dup_rows` duplicated rows exist and will be removed.
...
Here we answer the following: 1.Stratify the data by `hgb_quartiles` and report min, max, mean and sd of a `rbc`. 2.Stratify the by a `hgb_quatiles` and report min, max, mean and sd of a numeric column for a defined set of observations: +Only for persons with hgb \<= 10 +Only for persons with remission of inflamation +Only for persons older than around 40 years of age +Only for persons with more than 10% of monocytes in WBC 3.Use two categorical columns `active` and `remission` in the dataset to create a table
